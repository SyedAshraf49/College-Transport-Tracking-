...
const DriverLayout: React.FC = () => {
    const { user, logout, isTripActive, toggleTrip } = useAuth();
            ...
            <div className="w-full max-w-lg">
                ...
                <main className="space-y-6">
                    <Card className="text-center">
                        ...
                        <p className={`text-lg font-semibold mb-4 ${isTripActive ? 'text-green-400' : 'text-red-400'}`}>
                            Trip is {isTripActive ? 'Active' : 'Inactive'}
                        </p>
                        <button
                            onClick={handleToggleTrip}
                            className={`w-full py-4 rounded-lg text-white font-bold text-xl transition-transform transform hover:scale-105 ${isTripActive ? 'bg-red-500/70 hover:bg-red-500 border border-red-500/30' : 'bg-green-500/70 hover:bg-green-500 border border-green-500/30'}`}
                        >
                            <div className="flex items-center justify-center gap-2">
                                <PowerIcon className="w-6 h-6" />
                                <span>{isTripActive ? 'Stop Trip' : 'Start Trip'}</span>
                            </div>
                        </button>
                    </Card>
                    ...
                    <Card className="text-center">
                     ...
                     <Card>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                             <button className="w-full py-3 bg-yellow-500/70 text-white font-bold rounded-lg hover:bg-yellow-500 transition border-yellow-500/30">Traffic Delay</button>
                             <button className="w-full py-3 bg-orange-500/70 text-white font-bold rounded-lg hover:bg-orange-500 transition border-orange-500/30">Breakdown</button>
                        </div>
                    </Card>
                </main>
            </div>
