...
import { MOCK_BUSES, MOCK_ROUTES, MOCK_STOPS, MOCK_DRIVERS } from '../../constants';
...
const StudentLayout: React.FC = () => {
    const { user, logout, busLocations, isTripActive, notifications, addNotification, busLoopDurations } = useAuth();
    const student = user as Student;
    const tripActive = isTripActive(student.bus_id);
    ...
    const lastArrivalAtRef = React.useRef<number | null>(null);
    const lastPreAlertAtRef = React.useRef<number | null>(null);
    ...
    useEffect(() => {
        if (studentStop && busLocation && tripActive) {
            ...
            const loopDurationMs = busLoopDurations[student.bus_id] ?? 5 * 60 * 1000;
            const etaMs = (distance / 100) * loopDurationMs;
            const mockEta = Math.max(1, Math.round(etaMs / 60000));
            const arrivalThreshold = 1.5;
            const arrivedNow = distance <= arrivalThreshold;
            setHasArrived(arrivedNow);
            setEta(arrivedNow ? 0 : mockEta);
        } else {
            setEta(null);
            setHasArrived(false);
        }
    }, [busLocation, studentStop, tripActive, busLoopDurations, student.bus_id]);

    useEffect(() => {
        if (!tripActive) {
            lastArrivalAtRef.current = null;
            lastPreAlertAtRef.current = null;
        }
    }, [tripActive]);

    useEffect(() => {
        if (!studentStop || !busLocation || !bus || !tripActive) return;
        const distance = Math.sqrt(Math.pow(studentStop.latitude - busLocation.latitude, 2) + Math.pow(studentStop.longitude - busLocation.longitude, 2));
        const arrivalThreshold = 1.5;
        const preAlertThreshold = 4;
        const now = Date.now();
        const cooldownMs = busLoopDurations[bus.id] ?? 5 * 60 * 1000;

        const lastArrivalAt = lastArrivalAtRef.current ?? 0;
        const lastPreAlertAt = lastPreAlertAtRef.current ?? 0;
        const arrivalReady = now - lastArrivalAt >= cooldownMs;
        const preAlertReady = now - lastPreAlertAt >= cooldownMs && now - lastArrivalAt >= cooldownMs;

        if (distance <= arrivalThreshold && arrivalReady) {
            lastArrivalAtRef.current = now;
            addNotification({
                title: 'Bus Arrived',
                message: `${bus.bus_number} has arrived at ${studentStop.stop_name}.`,
                bus_id: bus.id,
                stop_id: studentStop.id,
                target_roles: ['student']
            });
            return;
        }

        if (distance <= preAlertThreshold && distance > arrivalThreshold && preAlertReady) {
            lastPreAlertAtRef.current = now;
            addNotification({
                title: 'Arriving Soon',
                message: `${bus.bus_number} is approaching ${studentStop.stop_name}.`,
                bus_id: bus.id,
                stop_id: studentStop.id,
                target_roles: ['student']
            });
        }
    }, [studentStop, busLocation, bus, addNotification, tripActive, busLoopDurations]);

    const filteredNotifications = React.useMemo(() => {
        return notifications.filter(notif => {
            if (!notif.target_roles.includes('student')) return false;
            if (notif.bus_id && notif.bus_id !== student.bus_id) return false;
            if (notif.stop_id && notif.stop_id !== student.stop_id) return false;
            return true;
        });
    }, [notifications, student.bus_id, student.stop_id]);

    const renderContent = () => {
        if (!busLocation) return <div>Loading bus data...</div>;
        switch(activeView) {
            case 'dashboard':
                return <StudentDashboard {...{student, bus, route, driver, studentStop, eta, stops, busLocation, isTripActive: tripActive, hasArrived}} />;
            case 'map':
                return <LiveTrackingMap {...{stops, busLocation, isTripActive: tripActive}} />;
            case 'notifications':
                return <Notifications notifications={filteredNotifications} />;
            case 'profile':
            ...
            <footer className="fixed bottom-0 left-0 right-0 bg-white/20 dark:bg-black/20 backdrop-blur-lg shadow-lg border-t border-white/20">
                <nav className="flex justify-around p-2">
                    ...
                    <NavItem
                        ...
                        badge={tripActive && eta !== null ? (hasArrived ? 'Arrived' : `Arriving in ~${eta} min`) : undefined}
                    />
...
const StudentDashboard: React.FC<{
    ...
    hasArrived: boolean,
}> = ({ student, bus, route, driver, studentStop, eta, stops, busLocation, isTripActive, hasArrived }) => (
            ...
            <div className="grid grid-cols-2 gap-4 text-center">
                <div>
                    ...
                    {isTripActive && eta !== null ? (
                         hasArrived ? (
                            <p className="text-3xl font-bold text-emerald-500">Arrived</p>
                         ) : (
